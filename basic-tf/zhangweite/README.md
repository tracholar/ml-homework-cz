...
12. dropout  
dropout作用于layer，他的作用是在网络训练的过程中（tf_is_train=true），随机使该layer的部分神经元输出为零，并且在更新参数时忽略这些零输出的神经元。这相当于是在训练过程中随机的简化模型，使得每个batch训练时的模型的结构都是变化的。在预测时(tf_is_train=false)，该layer全部神经元正常运行，但需要对其权值乘以dropout的概率p，以达到预测和训练输出值大小上相对一致的效果。  
...
神经网络在训练中，由于网络中参数变化会引起内部结点数据分布发生变化，被称作Internal Covariate Shift。这一现象会使得上层网络需要不断调整以适应输入分布的变化，降低网络训练速度。  
...
在训练过程中(tf_is_train=true)，每个batch的训练按上述操作即可，但需要存储所有特征的均值和方差。  
在预测过程中(tf_is_train=false)，输入数据归一化步骤的均值和方差即采用训练时统计的所有数据的均值和方差。  
