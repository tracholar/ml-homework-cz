...
def mlp_net(x_):
    with tf.name_scope('MLP'):
        in_x = tf.reshape(x_, [-1,784]) 
        l1 = tf.layers.dense(inputs = in_x, units = 512, activation = tf.nn.relu, name ='l1',
...
with tf.Session() as sess:
    ...
    ckpt_path = './tmp/mnist_mlp_dataset/model.ckpt'
