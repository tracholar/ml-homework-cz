...
class DeepFM(ModelMixin):
    def __init__(self,user_count,item_count,cate_count,cate_list):
        ...
        #############################################################
        fm_hidden=user_emb*item_emb+user_emb*cate_emb+item_emb*cate_emb+item_emb*cate_emb
        self.logits_fm=tf.reduce_sum(fm_hidden,axis=1)

        mlp_hidden = tf.layers.dense(inputs=mlp_input, units=32, activation=tf.nn.relu)

        self.logits_nn = tf.reduce_sum(tf.layers.dense(inputs=mlp_hidden, units=1, activation=None), axis=1)

        self.logits=self.logits_fm+self.logits_nn
