...
with tf.variable_scope('dqn'):
    h1 = tf.layers.dense(obs_placeholder, 64, activation=tf.nn.relu, kernel_initializer=tf.truncated_normal_initializer(stddev=0.7), name='hidden1')
    q = tf.layers.dense(h1, 2, name='q')

    tf.summary.histogram('dqn-h1', h1)
    tf.summary.histogram('dqn-q', q)

## set target net trainable to False
with tf.variable_scope('dqn_target'):
    h2 = tf.layers.dense(obs_new_placeholder, 64, activation=tf.nn.relu, kernel_initializer=tf.truncated_normal_initializer(stddev=0.7), trainable=False, name='hidden1')
    q_target = tf.layers.dense(h1, 2, trainable=False, name='q')
...
loss = tf.reduce_sum(tf.log1p((pred_q - target_q)**2)) + 1e-2*var_w
...
class ExpBuffer():
    def __init__(self, max_size=102400):
        self.buff = []
...
with tf.Session() as sess:
    ...
    for i in range(1024000):
        ...
        if (i+1) % 1024 == 0:
            summary, = sess.run([merge_op], feed_dict={
                obs_placeholder: obs,
                act_placeholder: act,
                rew_placeholder: rew,
                obs_new_placeholder: obsn,
            ...
            fwriter.add_summary(summary, global_step=istep)
